...
export function Dashboard() {
  const { accounts, accountByRole, completed, results, runStep, reset } =
    useDemoState();
  ...
  const buildTxContext = useMemo<BuildTxContext>(
    () => ({
      accountByRole,
      domainId,
      burnTxHash: resultByStep.get("p5-burn"),
    }),
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="px-6 py-2 bg-amber-500/8 border-b border-amber-500/20 flex items-center gap-2">
        ...
        <p className="text-[11px] text-amber-200/80">
          ...
          The Issuer account doubles as both the XRPL credential issuer and the
          EURF IOU issuer. In production these would be separate entities.
        </p>
      </div>
      ...
      <nav className="px-6 mt-6 flex gap-1 border-b border-slate-800">
        ...
        {PHASE_ORDER.map((p, i) => {
          ...
          const doneInPhase = stepsInPhase.filter((s) =>
            completed.has(s.id),
          ).length;
      ...
      {activeTab === "flow" ? (
        <main className="flex-1 px-6 py-5">
          <FlowDiagram completed={completed} onSelectStep={handleSelectStep} />
        </main>
      ) : (
            ...
            <div className="flex flex-col gap-4 max-h-[60vh] overflow-auto">
              {PHASE_ORDER.map((phase) => {
                ...
                return (
                  <div key={phase}>
                    <span
                      className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider font-semibold ring-1 mb-1.5 ${PHASE_BADGE[phase]}`}
                    >
                      {PHASE_LABELS[phase]}
                    </span>
                    <ul className="flex flex-col gap-1.5">
                      {phaseResults.map((r) => {
                        ...
                        const actorLabel = step
                          ? (accountByRole.get(step.actor)?.label ?? step.actor)
                          : null;
                        return (
                          <li
                            key={r.hash}
                          ...
                          >
                            <div className="flex items-center gap-1.5 text-[11px]">
                              ...
                              {step && Icon && (
                                <span
                                  className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] ring-1 shrink-0 ${ROLE_CHIP[step.actor]}`}
                                >
